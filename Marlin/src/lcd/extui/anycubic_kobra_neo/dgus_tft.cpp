...
#if ENABLED(ANYCUBIC_LCD_KOBRA_NEO)
...
//    #endif
    ...
    sdlog("LCD startup(): begin");
    sdlog("TFTSer.begin(115200) calling...");

    TFTSer.begin(115200);

    sdlog("TFTSer.begin(115200) done");

    // Signal Board has reset
    SendtoTFTLN((PGM_P)AC_msg_main_board_has_reset);
    sdlog("Sent AC_msg_main_board_has_reset to LCD");

    // Enable levelling and Disable end stops during print
    injectCommands(AC_cmnd_enable_leveling);

    sdlog("Sent AC_msg_ready to LCD");
    SendtoTFTLN((PGM_P)AC_msg_ready);

    // Force the display to the main page — it's likely stuck on the
    // bootloader's "updatefirmware" page from the SD-card flash process.
    // PAGE_MAIN = 1 for CHS, 121 for ENG.
    sdlog("Forcing page change to main page");
    ChangePageOfTFT(PAGE_MAIN);

    #if ACDEBUGLEVEL
      SERIAL_ECHOLNPGM("AC Debug Level ", ACDEBUGLEVEL);
    #endif
    sdlog("LCD startup() complete");
  }

  void DgusTFT::paramInit() {
    sdlog("paramInit() called");
    ...
    if(lcd_info.audio_on == true) {
      SERIAL_ECHOLN("lcd audio: ON");
    } else if(lcd_info.audio_on == false) {
  ...
  void DgusTFT::idleLoop()  {
    static uint32_t idle_log_count = 0;
    if (idle_log_count < 3) {
      #include "sdlog.h"
      char buf[40];
      snprintf(buf, sizeof(buf), "idleLoop() tick #%lu", (unsigned long)idle_log_count);
      sdlog(buf);
      idle_log_count++;
    }
    if (ReadTFTCommand()) {
...
//          SendtoTFTLN(AC_msg_paused); // enable continue button
          ...
          TERN_(HAS_FILAMENT_SENSOR, setFilamentRunoutState(false));
        }
      } break;
...
//        SendtoTFTLN(AC_msg_bed_heating);
  ...
  void DgusTFT::SendtoTFTLN(PGM_P str) {
    SendtoTFT(str);
    TFTSer.println();
  }
  ...
  void DgusTFT::LcdAudioSet(bool audio_on) {
           ...
           *p_u8 = data_buf[3];
           if((control_value&0x00FFFFFF) == 0x010072) { // startup last gif
             ...
             if(AC_printer_resuming_from_power_outage == printer_state) {
                ...
                SendTxtToTFT(ui8tostr3rj(getProgress_percent()), TXT_OUTAGE_RECOVERY_PROGRESS);
        ...
        void DgusTFT::page22_handle(void)   // print finish
        {
            ...
            switch (key_value)
            {
              ...
              case 1:  // OK to finish
              {
                ...
                // print time elapsed is cleared by the print timer itself
              }
              break;
